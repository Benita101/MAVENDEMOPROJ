...
public class BookingFormFillingPOM {

	int j=1, k=0;
	WebDriver driver;
	public BookingFormFillingPOM(WebDriver driver) {
		this.driver=driver;
	}
	
	By wrongEmailIdError = By.xpath("//div[@id='loginBlockError']");
	By insuranceTickBox = By.xpath("//input[@id='insurance_box']");
	By insuranceAccept = By.id("insurance_confirm");
	...
	{
		explicitwait.until(ExpectedConditions.visibilityOfElementLocated(continueBooking));
		
		driver.findElement(insuranceAccept).click();
